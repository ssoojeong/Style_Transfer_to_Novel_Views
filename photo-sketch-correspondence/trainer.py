...
def eval_knn(model, valid_loader, epoch, mem, args, writer, plot=False, save=None):
    """
    ...
    with torch.no_grad():
        ...
        for i, (images1, images2, _, _) in tqdm(enumerate(valid_loader), total=len(valid_loader)):
    ...
    _, indices = knn.kneighbors(features1, knn_n)
    ...
    os.makedirs('/userHome/userhome1/sojeong/psc/0603_warping_results', exist_ok=True)
    save_path = f'/userHome/userhome1/sojeong/psc/0603_warping_results/warping_results_n:{str(knn_n)}.txt'
    with open(save_path, 'a') as f:
        f.write('Epoch %d sketch2photo knn validation accuracy: %.6f' % (epoch, acc))
        f.write('\n')
        f.close() 
    ###
    ...
    _, indices = knn.kneighbors(features2, knn_n)
    ...
    ###
    with open(save_path, 'a') as f:
        f.write('Epoch %d photo2sketch knn validation accuracy: %.6f' % (epoch, acc))
        f.write('\n')
        f.close() 
    ###
